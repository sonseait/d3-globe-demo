...
export class Globe extends React.PureComponent {
  ...
  setupEarth = () => {
    this.emissiveMap = new THREE.TextureLoader().load(this.props.mapImageUrl);
  ...
  setupFlightsPathLines = () => {
    ...
    for (
      ...
      for (let segmentIndex = 0; segmentIndex < TOTAL_SEGMENT; segmentIndex++) {
        ...
        const segmentEndsAt = this.flightsPathSplines[flightIndex].getPoint(0);
  ...
  componentWillUnmount() {
    ...
    this.points.forEach((p) => p.material.dispose());
    try {
      if (this.requester) {
