...
export function ll2xyz(latitude: number, longitude: number, radius: number) {
...
export function latlongDistance(
  latitudeA: number,
  longitudeA: number,
  latitudeB: number,
  longitudeB: number
...
export function easeOutQuadratic(
  currentTime: number,
  startValue: number,
  change: number,
  duration: number
...
export function makeColorGradient(i: number, frequency: number, phase: number) {
  var center = 128;
  var width = 127;
...
export function _byte2Hex(n: number) {
