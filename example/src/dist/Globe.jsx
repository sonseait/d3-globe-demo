...
export class Globe extends React.PureComponent {
    constructor() {
        ...
        this.segments = new Float32Array(this.props.lines.length * 3 * 2 * segmentsTotal);
        this.points = [];
        ...
        this.setupEarth = (radius = 1) => {
            this.earth = new THREE.Mesh(new THREE.SphereGeometry(radius, 128, 128), new THREE.MeshPhongMaterial({
        ...
        this.setFlightTimes = (index) => {
            var flight = this.props.lines[index], distance = latlongDistance(flight[0], flight[1], flight[2], flight[3]), startTime = Date.now() + Math.floor(Math.random() * 1000 * 1), duration = Math.floor(distance * 1000 * 2);
        ...
        this.setupFlightsPathLines = () => {
            ...
            var material = new THREE.LineBasicMaterial({
                ...
                depthWrite: false,
                linewidth: 2,
            }), segmentBeginsAt, segmentEndsAt, colors = new Float32Array(this.props.lines.length * 3 * 2 * segmentsTotal), color = new THREE.Color(), index, beginsAtNormal, endsAtNormal;
            ...
            for (let f = 0; f < this.props.lines.length; f++) {
                for (let s = 0; s < segmentsTotal; s++) {
                    ...
                    color.setHSL(((this.props.lines[f][1] + 100) % 360) / 360, 1, 0.3 + beginsAtNormal * 0.2);
                    ...
                    color.setHSL(((this.props.lines[f][1] + 100) % 360) / 360, 1, 0.3 + endsAtNormal * 0.2);
                    colors[index + 3] = color.r;
                    colors[index + 4] = color.g;
                    colors[index + 5] = color.b;
                }
                const point1Value = this.flightsPathSplines[f].getPoint(0);
                const point1 = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.CircleGeometry(0.01, 64)), new THREE.LineBasicMaterial({
                    transparent: true,
                    opacity: 0.5,
                    color: 0xffffff,
                    linewidth: 1.5,
                ...
                const point2 = new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.CircleGeometry(0.01, 64)), new THREE.LineBasicMaterial({
                    transparent: true,
                    opacity: 0.5,
                    color: 0xffffff,
                    linewidth: 1.5,
            ...
            this.flightsPointCloudGeometry.addAttribute("position", new THREE.BufferAttribute(this.segments, 3));
            this.flightsPointCloudGeometry.addAttribute("color", new THREE.BufferAttribute(colors, 3));
            // Pack into the global varaible which is added to the scene later
            const flightsPathLines = new THREE.Line(this.flightsPointCloudGeometry, material, THREE.LinePieces);
        ...
        this.updateFlights = () => {
            ...
            for (f = 0; f < this.props.lines.length; f++) {
                if (Date.now() > this.flightsStartTimes[f]) {
                    easedValue = easeOutQuadratic(Date.now() - this.flightsStartTimes[f], 0, 1, this.flightsEndTimes[f] - this.flightsStartTimes[f]);
                    if (easedValue < 0 || isNaN(easedValue)) {
                        easedValue = 0;
                        this.setFlightTimes(f);
                    }
                    // đảo chiều
                    let isReverse = Date.now() - this.flightsStartTimes[f] >
                    ...
                    for (s = segmentsTotal - 1; s >= 0; s--) {
                        ...
                        // Begin this line segment.
                        segmentBeginsAt = this.flightsPathSplines[f].getPoint(Math.min(Math.max(beginsAtNormal, 0), 1));
                        this.segments[index + 0] = segmentBeginsAt.x;
                        this.segments[index + 1] = segmentBeginsAt.y;
                        this.segments[index + 2] = segmentBeginsAt.z;
                        // End this line segment.
                        segmentEndsAt = this.flightsPathSplines[f].getPoint(Math.min(Math.max(endsAtNormal, 0), 1));
                        this.segments[index + 3] = segmentEndsAt.x;
                        this.segments[index + 4] = segmentEndsAt.y;
                        this.segments[index + 5] = segmentEndsAt.z;
                    }
